...
const Leaderboard = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        Header: () => ((
            <span className='text-white'><span className='text-yellow-300'>Smash Cash </span>Earned</span>
        )),
        accessor: 'cashEarned',
        className: 'px-6 py-8 whitespace-no-wrap',
      },
      ...
      <table className="min-w-full" {...getTableProps()}>
        ...
        <tbody className='divide-y divide-gray-200' {...getTableBodyProps()}>
          {page.map((row) => {
            prepareRow(row);
            return (
              <tr className='dark:hover:bg-gray-800 transition' 
                {...row.getRowProps()}>
                {row.cells.map((cell) => (
                  <td
        ...
        {Array.from({ length: pageCount }, (_, index) => (
          <button
            ...
            className={`mx-1 px-3 py-2 rounded-md ${
              pageIndex === index ? 'bg-gray-700 text-white' : 'bg-gray-200 text-gray-700'
            }`}
